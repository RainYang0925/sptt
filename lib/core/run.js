...
var execute = {
  loadTestCases: function(done,setting){
    var {buildPath,type,name} = setting,
      pwd = process.cwd(),
      stamp = Date.now(),
      self = this;
    ...
    curpath = path.join(pwd,`${type}/steps`);
    fs.readdirSync(curpath,'utf8').forEach(function(name){
      ymlData.steps = _.assign(ymlData.steps,yml.safeLoad(fs.readFileSync(path.join(curpath,name),'utf8')));
    });

    // 指定具名的测试用例
    curpath = path.join(pwd,`${type}/testcases`);
    if(name && Array.isArray(name)){
      name.forEach(function(name){
        try{
          ymlData.testcases.push( yml.safeLoad(fs.readFileSync(path.join(curpath,name + '.yml'),'utf8')));
        }catch(e){
          ymlData.testcases.push( yml.safeLoad(fs.readFileSync(path.join(curpath,name + '.yaml'),'utf8')));
        }
      });
    }else{
      fs.readdirSync(curpath,'utf8').forEach(function(name){
        ymlData.testcases.push( yml.safeLoad(fs.readFileSync(path.join(curpath,name),'utf8')));
      });
    }

    ymlData.testcases.forEach(function(testcase){
      _.forEach(testcase,function(v,k){
        if(!v)
          return;
        var fileName = stamp + '_' + k;
        try{
          fs.writeFileSync(`${buildPath}/${fileName}.js`,self.runSeriesCases(k,testcase),'utf8');
        }catch(e){
  ...
  runSeriesCases: function(k,testcase){
    var self = this,
      config = testcase[k].config,
      ts = Object.keys(testcase[k]),
    ...
    ts.forEach(function(ca){
      if(!ca) return;
      code +=self.execTestcase(k,testcase,ca);
    });
  ...
  execTestcase: function(k,ts,testcaseDesc){
    var self = this,
      testcase = ts[k][testcaseDesc];
    // 测试用例代码
    var code = `
  ...
  execAtom: function(k,atomName,isOptional){
    ...
    var code = '',
      ...
      expectation_data = atom.expectation_data;
    ...
    switch(selectType){
      ...
      case 'css':
        code += `
        yield * (function*(){
          var el = yield $.findElementByCssSelector('${selector}');
          yield view.do({
            atomName: '${atomName}',
            el: el,
            selectorType: 'css',
    ...
    if(expectation_mark && expectation_type && !isOptional){
      let assertion = `
            switch('${expectation_type}'){
              ...
              case 'exist':
                ...
                break;
              case 'contain':
                let text = yield el.text();
                expect(text).to.contain('${expectation_data}');
                break;
            }
            log.info('sptt:','expectation of (${expectation_mark}) is ${expectation_type}');`;
      switch(expectationSelectType){
        ...
        case 'css':
          code += `yield* (function* checkExp(){
            var el = yield *$.findElementByCssSelector('${expectation_mark}');` +
            assertion +
            `
            // 切换上下文
            yield view.checkToNative('css');
            })();`;
          break;
        case 'className':
          // 当selector为className时，需要提供className的索引值
