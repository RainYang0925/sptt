...
var execute = {
  ...
  execAtom: function(k,atomName,isOptional){
    ...
    switch(selectType){
      ...
      case 'name':
        /*code += `
        yield *$.findElementByName('${selector}').${action}('${data}');
        `;*/
      ...
      default:
        // 支付操作无需制定UI元素
        if(action == 'pay'){
          let variable = data.url.match(/{{(\w+?)}}/)[1];
          code += `var promise = (function(){
            var request = require('request');
            return new Promise(function(resolve,reject){
              request.get({
                url: 'http://shopappserver.showjoy.net/Fake_Pay?orderNumber=' + ${variable},
                headers: {
                  cookie: 'tgc=bsK6KS0wQi;um_remember="5385980EA93F8265018C9FCD285B8ED7:2E175E4D7F73DDD96CC2524B965E12A6:CC1F08AA5F8A0FA68B8D3532EA3C68AC";'
                }
