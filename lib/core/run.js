  ...
  loadTestCases: function(done,setting){
    ...
    if(name && Array.isArray(name)){
      name.forEach(function(name){
        try{
          ymlData.testcases = _.assign(ymlData.testcases,yml.safeLoad(fs.readFileSync(path.join(curpath,name + '.yml'),'utf8')));
        }catch(e){
          ymlData.testcases = _.assign(ymlData.testcases,yml.safeLoad(fs.readFileSync(path.join(curpath,name + '.yaml'),'utf8')));
        }
      });
    }else{
      fs.readdirSync(curpath,'utf8').forEach(function(name){
