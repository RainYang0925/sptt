...
var clickWithCoordinate = function* (op){
  ...
  if(!(data.x || data.X) || !(data.y || data.Y)){
    log.warn('sptt:',`you must supply (x,y) information to click`);
    return;
  }

  try{
    log.info('sptt:',`begin click the coordinate(${data.x ? data.x : data.X},${data.y ? data.y : data.Y})`);
    ...
    log.error('sptt:',`click the coordinate (${data.x ? data.x : data.X},${data.y ? data.y : data.Y}) occur an error, ${e.stack}`);
    process.exit(1);
  }
};
